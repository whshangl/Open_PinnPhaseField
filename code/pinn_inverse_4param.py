...
G12 = dde.Variable(1.0)
G44 = dde.Variable(1.0)  
G44_ = dde.Variable(1.0)
...
FEM_data = np.loadtxt('./FEM_data/P1P2_25_11x11.txt', comments='%', delimiter=None)

xy_observ = FEM_data[:, 0:2]
...
P1_observ = FEM_data[:, 2:3]
P2_observ = FEM_data[:, 3:4]


#### add artificial noise
np.random.seed(123)
noise1 = np.random.normal(1, 0, P1_observ.shape[0]).reshape(P1_observ.shape)   ### change 0 to other values for different noise
np.random.seed(1234)
noise2 = np.random.normal(1, 0, P1_observ.shape[0]).reshape(P2_observ.shape)   ### change 0 to other values for different noise
...
store_path = Path('./Inverse_noise/param4_0/epoch')
store_path.mkdir(parents=True, exist_ok=True)


fname_var = "./Inverse_noise/varibles_4param_0.txt"
variable = dde.callbacks.VariableValue([G11, G12, G44, G44_], period=200, filename=fname_var, precision=4)
...
loss_weights = [
                ...
                100,   100                                   ## observations, change 100, 100 to 1, 1 for noisy data
                ] 
...
model.compile("adam", lr=1e-4, loss = 'MSE', loss_weights = loss_weights, external_trainable_variables=[G11, G12, G44, G44_])
...
model.compile("adam", lr=1e-5, loss = 'MSE', loss_weights = loss_weights, external_trainable_variables=[G11, G12, G44, G44_])
losshistory, train_state = model.train(iterations = 30000, display_every=1000, model_save_path = store_path, callbacks=[checkpointer, pde_resampler, variable])

model.compile("L-BFGS", loss = 'MSE', loss_weights = loss_weights, external_trainable_variables=[G11, G12, G44, G44_])
