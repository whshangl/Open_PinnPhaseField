...
store_path = Path('./time5/'+str(λ_F[0])+'_'+str(λ_F[1])+'_'+str(λ_F[2])+'_'+str(λ_F[3])+'_'+str(λ_F[4])+'/epoch')
...
dde.saveplot(losshistory, train_state, issave=True, isplot= False, output_dir=store_path)
...
##################################################################################
############################ store data of P1, P2 at t = 5 for series training ############################
###################################################################################
# Data_path = Path('./PINN_data')
# Data_path.mkdir(parents=True, exist_ok=True)

# model.compile("L-BFGS", loss = 'MSE', loss_weights = loss_weights)
# model.restore('./time5/1_1_10_100_100/epoch-100000.pt")
...
# pinn_P1P2_5 = np.zeros((X.shape[0],4))

# pinn_P1P2_5[:,0] = x_ 
# pinn_P1P2_5[:,1] = y_
# pinn_P1P2_5[:,2] = P1_pred
# pinn_P1P2_5[:,3] = P2_pred

# np.save('./PINN_data/pinn_P1P2_5.npy",pinn_P1P2_5)
...
##############################################################################################
############################ store data of P1, P2 at (1,0), (0,1), (-1,0),(0,-1) between t = 0 and t = 5 ############################
###############################################################################################
# Data_path = Path('./PINN_data')
# Data_path.mkdir(parents=True, exist_ok=True)
...
#     pinn_P1P2_temporal[:,2] = xyt[:,2]
#     pinn_P1P2_temporal[:,3] = P1_pinn
#     pinn_P1P2_temporal[:,4] = P2_pinn

#     np.save("./PINN_data/pinn_P1P2_0to5_" + str(xyt[0,0]) + "_" + str(xyt[0,1]) + ".npy",pinn_P1P2_temporal)
